...
import { getNgayPscMax, getDonViBaoCaoDoanhThu, getPhanTichPscDtTheoTram } from '../../services/didongApi';
import {
  ...
  Switch, FormControlLabel  , TextField , Box
} from '@mui/material';
...
export default function PhanTichDiDongPSCTheoTram_bts() {
...
const columnLabelMap = Object.fromEntries(
  ...
  const fetchDonviOptions = async () => {
    try {
      const data = await getDonViBaoCaoDoanhThu();
      setDonviOptions(data);
    } catch (error) {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const data = await getPhanTichPscDtTheoTram(
        formattedDate, 
        isKyTruoc ? 1 : 0, 
        selectedDonvi?.DONVI_ID ?? 0
      );
...
const wb = XLSX.utils.book_new();
  ...
  const columnGroups = [
    ...
    { label: 'MÃ TRẠM', keys: ['TEN_BTS'], color: '#d1c4e9' },
    { label: 'TÊN TRẠM', keys: ['TEN_TRAM'], color: '#f8bbd0' },
...
//sx={{ p: 2, pb: 8 }}
  return (
      ...
      <Typography variant="h5" gutterBottom align="center" color='blue' fontWeight="bold">
        DOANH THU SỐ LƯỢNG TB PSC CHI TIẾT THEO BTS
      </Typography>
      ...
      <LocalizationProvider dateAdapter={AdapterDateFns} adapterLocale={vi}>
        ...
        <Grid item>
          <Autocomplete
            ...
            getOptionLabel={(option) => option.TEN_DV || ''}
            value={selectedDonvi}
            onChange={(e, newVal) => setSelectedDonvi(newVal)}
            renderInput={(params) => <TextField {...params} label="Chọn đơn vị" variant="outlined" sx={{ minWidth: 250 }} />}
          />
        </Grid>
          <Grid item>
