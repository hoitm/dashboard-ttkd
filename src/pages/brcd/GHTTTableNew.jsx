...
const headerGroups = [
  { label: "STT", span: 1, color: "from-gray-300 to-gray-400",  width: "60px", rowSpan: 2, startColumn: 0 },
  { label: "Tên đơn vị, cá nhân", span: 1, color: "from-gray-300 to-gray-400",  width: "200px", rowSpan: 2, startColumn: 1 },
 // { label: "Tổng Fiber chuyển xuống Offfline, Online", span: 1, color: "from-purple-100 to-purple-400", rowSpan: 2, groupIndex: 0, startColumn: 0, endColumn: 0 },
  { label: "ĐÃ THỰC HIỆN (Fiber)",     span: 7, color: "from-amber-100 to-amber-200", groupIndex: 1, startColumn: 0, endColumn: 7},
  { label: "CHƯA THỰC HIỆN (Fiber)", span: 6, color: "from-blue-100 to-blue-200", groupIndex: 2, startColumn: 7 ,endColumn: 13 },
...
//"SL chuyển TS" , 	"Tỷ lệ chuyển TS (T<10%)",	"SL Gia hạn",	"Tỷ lệ Gia hạn (T>60%)",	"SL Thanh lý",	"Tỷ lệ Thanh lý (T<1%)"
// "Tổng Số lượng chưa thực hiện",	"Tổng tỷ lệ chưa thực hiện", "Số lượng",	"Số lượng	Phiếu còn 1 ngày đến hạn",	"Phiếu đã quá 72h",
 

const subHeaders = [ "Tổng Fiber chuyển xuống Offfline, Online",
  "SL chuyển TS" , 	"Tỷ lệ chuyển TS (T<10%)",	"SL Gia hạn",	"Tỷ lệ Gia hạn (T>60%)",	"SL Thanh lý",	"Tỷ lệ Thanh lý (T<1%)",
 "Tổng Số lượng chưa thực hiện",	"Tổng tỷ lệ chưa thực hiện", "Số lượng Gia hạn", "Số lượng",	"Số lượng	Phiếu còn 1 ngày đến hạn",	"Phiếu đã quá 72h",
...
/*
    "stt": 1,
        "TTVT_65_1": "TTVT DTH",
        "TEN_NV_NHANPHIEU": "Dương Bình Hiển",
        "tong_phieu_cth": 0,
        "Fiber": 0,
        "tong_phieu_cts": 0,
        "Fiber_cskh_ob": 0,
...
// Helper function for getting values from row data
const getValues = (data) => [
    data.tong_phieu_cth, 
    data.Fiber, 
    data.tong_phieu_cts,
    data.Fiber_cskh_ob, 
    `${data.tyle_tong_phieu_cts}%`,
    ...
    data.tong_phieu_gh1, data.Fiber_cskh_tn,
    data.tyle_tong_phieu_tn, data.chua_thuchien,
    `${data.tyle_chua_lam}%`
];
...
// Helper function for border styles
...
export default function GHTTTableNew() {
  ...
  const fetchData = async () => {
    try {
      ...
      const response = await axios.post('https://ttkd.vnptphuyen.vn:4488/api/AppOnline/web-dhsxkd-ghtt_thnv_mau_2', {
  ...
  const exportToExcel = () => {
    ...
    // Generate Excel file
    XLSX.writeFile(wb, "GHTT_Report_New.xlsx");
  };
